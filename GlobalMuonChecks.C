                      ...
                      const std::string o2sim_KineFile = "o2sim_Kine.root")
{
  ...
  std::map<int,const char *> TH2Names {
    ...
    {kGMTrackDeltaXYVertex4plus, "Global Muon Tracks Vertex at Z = 0 Pt4plus"},
  ...
  std::map<int,const char *> TH1Names {
    ...
    {kGMTrackDeltaPhi0_1,"Global Muon Tracks Fitted Phi at Vertex [rad] (pt < 1)"},
    {kGMTrackDeltaPhi1_4,"Global Muon Tracks Fitted Phi at Vertex [rad] (1 < pt < 4)"},
    {kGMTrackDeltaPhi4plus,"Global Muon Tracks Fitted Phi at Vertex [rad] (pt > 4)"},
    {kGMTrackDeltaPhiDeg,"Global Muon Tracks Fitted Phi at Vertex [deg]"},
    {kGMTrackDeltaPhiDeg0_1,"Global Muon Tracks Fitted Phi at Vertex [deg] (pt < 1)"},
    {kGMTrackDeltaPhiDeg1_4,"Global Muon Tracks Fitted Phi at Vertex [deg] (1 < pt < 4)"},
    {kGMTrackDeltaPhiDeg4plus,"Global Muon Tracks Fitted Phi at Vertex [deg] (pt > 4)"},
  ...
  std::map<int, std::array<double,3>> TH1Binning {
    ...
    {kGMTrackDeltaPhiDeg, {1000, TMath::RadToDeg()*deltaphiMin, TMath::RadToDeg()*deltaphiMax}},
    {kGMTrackDeltaPhiDeg0_1, {1000, TMath::RadToDeg()*deltaphiMin, TMath::RadToDeg()*deltaphiMax}},
    {kGMTrackDeltaPhiDeg1_4, {1000, TMath::RadToDeg()*deltaphiMin, TMath::RadToDeg()*deltaphiMax}},
    {kGMTrackDeltaPhiDeg4plus, {1000, TMath::RadToDeg()*deltaphiMin, TMath::RadToDeg()*deltaphiMax}},
  ...
  TEfficiency* qMatchEff = new TEfficiency("QMatchEff","Charge Match;p_t [GeV];#epsilon",20,0,10);
  ...
  TEfficiency* pairedMCHTracksEff = new TEfficiency("PairingEff","Paired_tracks;p_t [GeV];#epsilon",20,0,10);
  TEfficiency* globalMuonCorrectMatchRatio = new TEfficiency("Correct_Match_Ratio"," CorrectMatchRatio (nCorrectMatches/NGlobalMuonTracks);p_t [GeV];#epsilon",20,0,10);
  TEfficiency* globalMuonCombinedEff = new TEfficiency("Global_Matching_Efficiency","Global_Matching_Efficiency (nCorrectMatches/NMCHTracks);p_t [GeV];#epsilon",20,0,10);
  TEfficiency* closeMatchEff = new TEfficiency("Close_Match_Eff","Close Matches;p_t [GeV];#epsilon",20,0,10);
  ...
  for (int iEvent = 0 ; iEvent < numberOfEvents ; iEvent++ ) {
    ...
    if (0) for(auto& gmTrack: trackGMVec) {
    ...
    for(auto& gmTrack: trackGMVec) {
      ...
      if(iEvent == label[0].getEventID()) {
        if(DEBUG_VERBOSE) {
          //std::cout << "  Global Track ID = " <<  iTrack << " ; MFTMatchID = " << bestMFTTrackMatchID << " SourceID = " << label[0].getSourceID() << " ; EventID = " << label[0].getEventID() << ":  trackID = " << label[0].getTrackID() << " ; isFake = " << label[0].isFake() << " Label: ";
          std::cout << "  Global Track ID = " <<  iTrack << " ; MFTMatchID = " << bestMFTTrackMatchID  << " Label: ";
          label[0].print();

          //std::cout << "        bestMFTTrackMatchID = " << bestMFTTrackMatchID << " / labelMFTBestMatch = ";
          //labelMFTBestMatch[0].print();
        }
        if (gmTrack.closeMatch()) nCloseMatches++;
        ...
        if(label[0].isCorrect()) { // Correct match track: add to histograms
          ...
          if (TDatabasePDG::Instance()->GetParticle(pdgcode_MC)) {
            ...
            if (DEBUG_VERBOSE) std::cout << "      => " <<  TDatabasePDG::Instance()->GetParticle(pdgcode_MC)->GetName() <<  "\n";
          }
          ...
          auto invQPtChi2 = d_invQPt*d_invQPt/sqrt(gmTrack.getCovariances()(4,4));
          ...
          //std::cout << "DeltaX / sigmaX = " << dx/sqrt(gmTrack.getCovariances()(0,0)) << std::endl;
          TH1Histos[kGMTrackDeltaXErr]->Fill(dx/sqrt(gmTrack.getCovariances()(0,0)));
          //std::cout << "DeltaY / sigmaY = " << dy/sqrt(gmTrack.getCovariances()(1,1)) << std::endl;
          TH1Histos[kGMTrackDeltaYErr]->Fill(dy/sqrt(gmTrack.getCovariances()(1,1)));
          //std::cout << "DeltaPhi / sigmaPhi = " << d_Phi/sqrt(gmTrack.getCovariances()(2,2)) << std::endl;
          TH1Histos[kGMTrackDeltaPhiErr]->Fill(d_Phi/sqrt(gmTrack.getCovariances()(2,2)));
          //std::cout << "DeltaTanl / sigmaTanl = " << d_tanl/sqrt(gmTrack.getCovariances()(3,3)) << std::endl;
          TH1Histos[kGMTrackDeltaTanLErr]->Fill(d_tanl/sqrt(gmTrack.getCovariances()(3,3)));
          //std::cout << "DeltaPt / sigmaPt = " << d_Pt/sqrt(gmTrack.getCovariances()(4,4)) << std::endl;
          TH1Histos[kGMTrackDeltainvQPtErr]->Fill(d_invQPt/sqrt(gmTrack.getCovariances()(4,4)));
          ...
          TH2Histos[kGMTrackInvPtResolution]->Fill(Pt_MC,(1.0/Pt_fit-1.0/Pt_MC)*Pt_MC);
  ...
  TH1Histos[kGMTrackQ1_4]->SetTitle(Form("nChargeMatch = %d (%.2f%%)", nChargeMatch1_4, 100.*nChargeMatch1_4/(nChargeMiss1_4+nChargeMatch1_4)));
  ...
  globalMuonCorrectMatchRatio->SetTitle(Form("Correct_Match_Ratio = %.2f%%", 100.*nCorrectMatchGMTracks/(nRecoGMTracks)));
  closeMatchEff->SetTitle(Form("Close_Match_=_%.2f%%", 100.*nCloseMatches/(nRecoGMTracks)));
  ...
  auto covariances_summary = summary_report_3x2(*TH1Histos[kGMTrackDeltaXErr],
    ...
    *TH2Histos[kGMTrackQPRec_MC],
    ...
    Form("%.2f%%", 100.0*TH1Histos[kGMTrackDeltainvQPtErr]->Integral()/TH1Histos[kGMTrackDeltainvQPtErr]->GetEntries()),
    Form("%.2f%%", 100.0*TH1Histos[kGMTrackDeltaYErr]->Integral()/TH1Histos[kGMTrackDeltaYErr]->GetEntries()),
    Form("%.2f%%", 100.0*TH1Histos[kGMTrackDeltaTanLErr]->Integral()/TH1Histos[kGMTrackDeltaTanLErr]->GetEntries()),
    Form("%.2f%%", 100.0*TH2Histos[kGMTrackQPRec_MC]->Integral()/TH2Histos[kGMTrackQPRec_MC]->GetEntries())
  );
  ...
  auto param_summary_diff_pt = summary_report_3x3(*TH1Histos[kGMTrackDeltaX0_1],
    *TH1Histos[kGMTrackDeltaTanl0_1],
    *TH1Histos[kGMTrackDeltaPhiDeg0_1],
    *TH1Histos[kGMTrackDeltaX1_4],
    *TH1Histos[kGMTrackDeltaTanl1_4],
    *TH1Histos[kGMTrackDeltaPhiDeg1_4],
    *TH1Histos[kGMTrackDeltaX4plus],
    ...
    *TH1Histos[kGMTrackDeltaPhiDeg4plus],
    ...
    Form("%.2f%%", 100.0*TH1Histos[kGMTrackDeltaPhiDeg0_1]->Integral()/TH1Histos[kGMTrackDeltaPhiDeg0_1]->GetEntries()),
    Form("%.2f%%", 100.0*TH1Histos[kGMTrackDeltaX1_4]->Integral()/TH1Histos[kGMTrackDeltaX1_4]->GetEntries()),
    Form("%.2f%%", 100.0*TH1Histos[kGMTrackDeltaTanl1_4]->Integral()/TH1Histos[kGMTrackDeltaTanl1_4]->GetEntries()),
    Form("%.2f%%", 100.0*TH1Histos[kGMTrackDeltaPhiDeg1_4]->Integral()/TH1Histos[kGMTrackDeltaPhiDeg1_4]->GetEntries()),
    Form("%.2f%%", 100.0*TH1Histos[kGMTrackDeltaX4plus]->Integral()/TH1Histos[kGMTrackDeltaX4plus]->GetEntries()),
    Form("%.2f%%", 100.0*TH1Histos[kGMTrackDeltaTanl4plus]->Integral()/TH1Histos[kGMTrackDeltaTanl4plus]->GetEntries()),
    Form("%.2f%%", 100.0*TH1Histos[kGMTrackDeltaPhiDeg4plus]->Integral()/TH1Histos[kGMTrackDeltaPhiDeg4plus]->GetEntries())
  );


  auto pt_resolution = summary_report(*TH2Histos[kGMTrackPtResolution],
    *TH2Histos[kGMTrackQPRec_MC],
    *PtRes_Profile,
    *qMatchEff,
    "Pt Summary",
  ...
  auto invpt_resolution = summary_report(*TH2Histos[kGMTrackInvPtResolution],
    *TH2Histos[kGMTrackQPRec_MC],
  ...
  auto vertexing_resolution = summary_report(*TH2Histos[kGMTrackDeltaXYVertex],
    *TH1Histos[kGMTrackDeltaX],
    *DeltaX_Error,
    *TH1Histos[kGMTrackDeltaPhiDeg],
    ...
    Form("-"),
    Form("%.2f%%", 100.0*TH1Histos[kGMTrackDeltaPhiDeg]->Integral()/TH1Histos[kGMTrackDeltaPhiDeg]->GetEntries())
  );

  auto vertexing_resolution0_1 = summary_report(*TH2Histos[kGMTrackDeltaXYVertex0_1],
    *TH1Histos[kGMTrackDeltaX0_1],
    *TH1Histos[kGMTrackDeltaTanl0_1],
    *TH1Histos[kGMTrackDeltaPhiDeg0_1],
  ...
  auto vertexing_resolution1_4 = summary_report(*TH2Histos[kGMTrackDeltaXYVertex1_4],
    *TH1Histos[kGMTrackDeltaX1_4],
    *TH1Histos[kGMTrackDeltaTanl1_4],
    *TH1Histos[kGMTrackDeltaPhiDeg1_4],
  ...
  auto vertexing_resolution4plus = summary_report(*TH2Histos[kGMTrackDeltaXYVertex4plus],
    *TH1Histos[kGMTrackDeltaX4plus],
    *TH1Histos[kGMTrackDeltaTanl4plus],
    *TH1Histos[kGMTrackDeltaPhiDeg4plus],
  ...
  std::cout << " Tanl_mean = " << TH1Histos[kGMTrackDeltaTanl]->GetMean() << std::endl;
  std::cout << " Tanl_StdDev = " << TH1Histos[kGMTrackDeltaTanl]->GetStdDev() << std::endl;
  std::cout << " Tanl_StdDev(pt<1) = " << TH1Histos[kGMTrackDeltaTanl0_1]->GetStdDev() << std::endl;
  std::cout << " Tanl_StdDev(1<pt<4) = " << TH1Histos[kGMTrackDeltaTanl1_4]->GetStdDev() << std::endl;
  std::cout << " Tanl_StdDev(pt>4) = " << TH1Histos[kGMTrackDeltaTanl4plus]->GetStdDev() << std::endl;
  std::cout << " Phi_mean = " << TH1Histos[kGMTrackDeltaPhi]->GetMean() << std::endl;
  std::cout << " Phi_StdDev = " << TH1Histos[kGMTrackDeltaPhi]->GetStdDev() << std::endl;
  std::cout << " Phi_StdDev(pt<1) = " << TH1Histos[kGMTrackDeltaPhi0_1]->GetStdDev() << std::endl;
  std::cout << " Phi_StdDev(1<pt<4) = " << TH1Histos[kGMTrackDeltaPhi1_4]->GetStdDev() << std::endl;
  std::cout << " Phi_StdDev(pt>4) = " << TH1Histos[kGMTrackDeltaPhi4plus]->GetStdDev() << std::endl;
  std::cout << " Phi_meanDeg = " << TH1Histos[kGMTrackDeltaPhiDeg]->GetMean() << std::endl;
  std::cout << " Phi_StdDevDeg = " << TH1Histos[kGMTrackDeltaPhiDeg]->GetStdDev() << std::endl;
  std::cout << " Phi_StdDevDeg(pt<1) = " << TH1Histos[kGMTrackDeltaPhiDeg0_1]->GetStdDev() << std::endl;
  std::cout << " Phi_StdDevDeg(1<pt<4) = " << TH1Histos[kGMTrackDeltaPhiDeg1_4]->GetStdDev() << std::endl;
  std::cout << " Phi_StdDevDeg(pt>4) = " << TH1Histos[kGMTrackDeltaPhiDeg4plus]->GetStdDev() << std::endl;
  std::cout << " DeltaX_mean = " << TH1Histos[kGMTrackDeltaX]->GetMean() << std::endl;
  std::cout << " DeltaX_StdDev = " << TH1Histos[kGMTrackDeltaX]->GetStdDev() << std::endl;
  std::cout << " DeltaX_StdDev(pt<1) = " << TH1Histos[kGMTrackDeltaX0_1]->GetStdDev() << std::endl;
  std::cout << " DeltaX_StdDev(1<pt<4) = " << TH1Histos[kGMTrackDeltaX1_4]->GetStdDev() << std::endl;
  std::cout << " DeltaX_StdDev(pt>4) = " << TH1Histos[kGMTrackDeltaX4plus]->GetStdDev() << std::endl;
  std::cout << " DeltaY_mean = " << TH1Histos[kGMTrackDeltaY]->GetMean() << std::endl;
  std::cout << " DeltaY_StdDev = " << TH1Histos[kGMTrackDeltaY]->GetStdDev() << std::endl;
  ...
  std::cout << " nTrackMatch = " << nCorrectMatchGMTracks << " (" << 100.*nCorrectMatchGMTracks/(nRecoGMTracks) << "%)" << std::endl;
  std::cout << "---------------------------------------------------------------------------" << std::endl;
  ...
  std::cout << " ==> "<< nCloseMatches  << "close matches - correct MFT track in search window" << " (" << 100.*nCloseMatches/(nRecoGMTracks) << "%)"<< std::endl;
  std::cout << " ==> "<< nCorrectMatchGMTracks << " Correct Match Global Muon Tracks" << " (Correct_Match_Ratio = " << 100.*nCorrectMatchGMTracks/(nRecoGMTracks) << "%)" << " (eff. = " << 100.*nCorrectMatchGMTracks/(nMCHTracks) << "%)"<< std::endl;
  ...
  std::cout << "matching_helper.nCorrectMatches = " << matching_helper.nCorrectMatches << std::endl;
  std::cout << "matching_helper.nCloseMatches = " << matching_helper.nCloseMatches << std::endl;
