...
std::vector<float> getVariables(const GlobalFwdTrack &mchTrack,
  ...
  std::vector<float> input_tensor_values{
      MFT_X,   MFT_Y,     MFT_Phi,    MFT_Tanl,    MFT_InvQPt,    MCH_X,
      MCH_Y,   MCH_Phi,   MCH_Tanl,   MCH_InvQPt,  MFT_TrackChi2, Delta_X,
      Delta_Y, Delta_Phi, Delta_Tanl, Delta_InvQPt};
...
Ort::Experimental::Session session = Ort::Experimental::Session(
    env, model_file, session_options); // access experimental components via the
...
MatchingFunc_t *getMatchingFunction() { return &matchONNX; }

CutFunc_t sameCharge = [](const GlobalFwdTrack &mchTrack,
                          const TrackParCovFwd &mftTrack) -> bool {
  // Example of external MFTMCH candidate cut function
  return mchTrack.getCharge() == mftTrack.getCharge();
};

CutFunc_t *getCutFunction() { return &sameCharge; }
